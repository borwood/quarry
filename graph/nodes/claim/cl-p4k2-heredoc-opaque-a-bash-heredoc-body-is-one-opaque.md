...
This closes the residue cl-zj2c named in its closing paragraph — that paragraph now describes ground that is covered.

AMENDED AT THE it-dprv LANDING (2026-08-21, the dispatcher's hand): this paragraph named the newline hole as what remained open in this parser. It is closed too. cl-v2vh ends a command at an unquoted newline, with a continuation exception one word wide — a lone `\` or lone backtick standing as its own word before the newline, narrowed by measurement rather than taste, because a rule reading any trailing backslash turns a Windows destination path into a plausible target nobody wrote. The heredoc arm here runs ahead of that boundary, so a body is still taken whole before any newline inside it can end a command; cl-v2vh leans on that ordering.

The named residue that remains in this parser is the glued continuation form (a trailing `\` fused to the word before it), which parses to nothing either way because bash's own joining fuses the two halves into a single implausible word. This body also carried doubled apostrophes throughout from a shell-quoting mishap at its minting; repaired in the same act.
